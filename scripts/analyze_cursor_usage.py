...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog='analyze_cursor_usage')
...
def build_service(include_correlation: bool, include_tokens: bool) -> CursorAnalyticsService:
    store = CursorSessionStore()
    capture_builder = CursorTranscriptCaptureBuilder(store)
    token_estimator = TranscriptTokenEstimator() if include_tokens else None
    if not include_correlation:
        return CursorAnalyticsService(store, capture_builder=capture_builder, token_estimator=token_estimator)
    analytics_store = JsonlAnalyticsStore(AnalyticsSettings.from_env())
    correlation = AnalyticsCorrelationService(analytics_store)
    return CursorAnalyticsService(
        store,
