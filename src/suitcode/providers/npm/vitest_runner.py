    ...
    def list_test_files(self, package_root: str) -> tuple[str, ...]:
        ...
        try:
            completed = subprocess.run(
                [str(self._executable_path), "list", "--filesOnly", "--run"],
                cwd=package_path,
            ...
            raise ValueError(
                f"vitest test listing timed out for package `{package_root}` in `{self._repository_root}`"
                f"{': ' + output if output else ''}"
            ) from exc
        if completed.returncode != 0:
            raise ValueError(
                "vitest test listing failed for package "
                f"`{package_root}` in `{self._repository_root}`: {completed.stderr.strip() or completed.stdout.strip()}"
            )
        files = set()
        for raw_line in completed.stdout.splitlines():
            ...
            if not candidate.exists() or not candidate.is_file():
                raise ValueError(
                    f"vitest test listing returned an invalid file path `{line}` for package `{package_root}`"
                )
